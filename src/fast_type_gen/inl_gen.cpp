...
public:
  ...
  virtual void visit(const byte_vector_field_instruction* inst, void*) override
  {
  ...
  virtual void visit(const int32_vector_field_instruction* inst, void*) override
  {
    out_ << "ext_cref< int32_vector_cref, "  << get_operator_tag(inst) << ", " << get_properties_type(inst) << ">";
  }

  virtual void visit(const uint32_vector_field_instruction* inst, void*) override
  {
    out_ << "ext_cref< uint32_vector_cref, "  << get_operator_tag(inst) << ", " << get_properties_type(inst) << ">";
  }

  virtual void visit(const int64_vector_field_instruction* inst, void*) override
  {
    out_ << "ext_cref< int64_vector_cref, "  << get_operator_tag(inst) << ", " << get_properties_type(inst) << ">";
  }

  virtual void visit(const uint64_vector_field_instruction* inst, void*) override
  {
...
template <typename T>
struct visitor_print_proxy {
  T* inst_;
  visitor_print_proxy(T* inst)
    : inst_(inst)
  {
  }
};
...
template <typename T>
inline visitor_print_proxy<T>
print_visitor(T* inst)
{
...
void inl_gen::visit(const mfast::group_field_instruction* inst, void* pIndex)
{
  ...
  std::size_t index = pIndex == nullptr ? 0 : *static_cast<std::size_t*>(pIndex);
  ...
  if (inst->ref_instruction() == nullptr && !embed_only_dyn_tempateref) {
    ...
    out_ << "inline\n"
         ...
         << "inline\n"
         << cref_type_name << "::" << name << "_cref(\n"
         << "  typename std::enable_if<std::is_same<typename T::cref_type, " << name << "_cref>::value, const mfast::value_storage*>::type storage_array,\n"
         << "  const T* instruction)\n"
    ...
    out_ << "inline\n"
         ...
         << "  typename std::enable_if<std::is_same<typename T::cref_type, " << name << "_cref>::value, mfast::value_storage*>::type storage_array,\n"
         << "  const T*          instruction)\n"
    ...
    if (!pIndex)
    {
      out_ << "inline\n"
           ...
           << "}\n\n"
           << "inline\n"
           << name << "::" << name << "(" << name << "&& other) BOOST_NOEXCEPT\n"
           ...
           << "}\n\n"
           << "inline "<< name << "&\n"
           << name << "::operator =(" << name << "&& other) BOOST_NOEXCEPT\n"
           << "{\n"
...
void inl_gen::visit(const mfast::template_instruction* inst, void*)
{
  ...
  out_ << "template <typename Visitor>\n"
       << "inline void " << name << "_cref::accept(Visitor& " << print_visitor(inst) << ")\n"
       << "{\n"
       << "  using namespace mfast;\n";
  ...
  out_ << "template <typename Visitor>\n"
       << "inline void " << name << "_mref::accept(Visitor& " << print_visitor(inst) << ")\n"
       << "{\n"
       << "  using namespace mfast;\n";
