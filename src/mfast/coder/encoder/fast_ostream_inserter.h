...
namespace mfast
{
  ...
  {
    strm.encode(cref.c_str(),static_cast<uint32_t>( cref.size()), cref.instruction(), cref.instruction()->is_nullable());
    return strm;
  }
  ...
  {
    strm.encode(cref.begin(), static_cast<uint32_t>(cref.size()), cref.instruction(), cref.instruction()->is_nullable());
    return strm;
  }

  inline fast_ostream& operator << (fast_ostream& strm, const decimal_cref& cref)
  {
    strm.encode(cref.exponent(), cref.absent(), cref.instruction()->is_nullable());
    if (cref.present()) {
