...
namespace mfast
{
  namespace SQLite3
  {
    ...
    field_cref aggregate_inserter::insert(const aggregate_cref& ref, const field_cref& foreign_key)
    {
      int rowid = 0;
      ...
      if (first_vistor.nested_sequence_pass_needed()) {
        second_pass_visitor second_vistor(core_, ref.instruction(), foreign_key.absent() ? 1 : 2);
