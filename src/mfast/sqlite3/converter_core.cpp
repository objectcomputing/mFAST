...
namespace mfast
{
  namespace SQLite3
  {
    ...
    struct tag_binding;
    struct binding_info
      : public boost::error_info<tag_binding,std::string>
    {
    ...
    converter_core::converter_core(const char*        dbfile,
                                   const field_masks& mask )
      : field_masks(mask)
    {
      if (sqlite3_open_v2(dbfile, &db_, SQLITE_OPEN_CREATE | SQLITE_OPEN_READWRITE, 0)) {
        BOOST_THROW_EXCEPTION(sqlite3_error(db_, "sqlite3_open") << boost::errinfo_file_name(dbfile));
      }
    }
    ...
    void converter_core::add(const template_instruction* inst, bool to_create_tables)
    {
      ...
      if (to_create_tables) {
        if (sqlite3_exec(db_,
                         creator.create_statements().c_str(),
                         0,
                         0,
                         0) != SQLITE_OK)
        {
      ...
      BOOST_FOREACH (tables_creator::insert_map_t::const_reference v, the_map)
      {
        ...
        if (sqlite3_prepare_v2(db_,
                               v.second.insert_item_stmt.c_str(),
                               v.second.insert_item_stmt.size(),
                               &info.insert_stmt_,
                               0) != SQLITE_OK)
        {
          BOOST_THROW_EXCEPTION(sqlite3_error(db_, "sqlite3_prepare_v2") << statement_info(v.second.insert_item_stmt.c_str()));
        }
...
#endif
          if (sqlite3_prepare_v2(db_,
                                 v.second.find_key_stmt.c_str(),
                                 v.second.find_key_stmt.size(),
                                 &info.find_key_stmt_,
                                 0) != SQLITE_OK)
          {
            BOOST_THROW_EXCEPTION(sqlite3_error(db_, "sqlite3_prepare_v2")<< statement_info(v.second.find_key_stmt.c_str()));
          }
        }
    ...
    struct unsupported_group_error
      : error {};


    const template_info& converter_core::info(const group_field_instruction* inst) const
    {
      unsigned id = (inst->ref_instruction() != 0) ? inst->ref_instruction()->id() : inst->id();
      template_infos_t::const_iterator itr = infos_.find(id);
      if (itr == infos_.end())
        BOOST_THROW_EXCEPTION(unsupported_group_error() << field_name_info(inst->name()));
    ...
    void converter_core::bind(const template_info& info, unsigned column,  int64_t value) const
    {
    ...
    void converter_core::bind(const template_info& info, unsigned column,  const decimal_cref& ref) const
    {
    ...
    void converter_core::bind(const template_info& info, unsigned column,  const char* str) const
    {
      info.binding_[column] = str;

      if (sqlite3_bind_text(info.insert_stmt_,column,str, -1, SQLITE_STATIC)!= SQLITE_OK)
        BOOST_THROW_EXCEPTION(sqlite3_error(db_, "sqlite3_bind_text"));
      if (info.find_key_stmt_)
        if (sqlite3_bind_text(info.find_key_stmt_,column,str, -1, SQLITE_STATIC)!= SQLITE_OK)
          BOOST_THROW_EXCEPTION(sqlite3_error(db_, "sqlite3_bind_text"));
    }

    void converter_core::bind(const template_info& info, unsigned column, const byte_vector_cref& ref) const
    {
      std::stringstream strm;
      ...
      if (sqlite3_bind_blob(info.insert_stmt_,column,ref.data(), ref.size(), SQLITE_STATIC)!= SQLITE_OK)
        BOOST_THROW_EXCEPTION(sqlite3_error(db_, "sqlite3_bind_blob"));
      if (info.find_key_stmt_)
        if (sqlite3_bind_blob(info.find_key_stmt_,column,ref.data(), ref.size(), SQLITE_STATIC)!= SQLITE_OK)
          BOOST_THROW_EXCEPTION(sqlite3_error(db_, "sqlite3_bind_blob"));
    }

    void converter_core::bind_null(const template_info& info, unsigned column) const
    {
    ...
    public:
      field_cref_binder(const converter_core& core, const template_info& info, unsigned column)
        : core_(core)
        , info_(info)
        , column_(column)
      {
      ...
      {
        core_.bind(info_, column_, static_cast<value_storage*>(storage)->get<int64_t>() );
      }

      void visit(const ascii_field_instruction* , void* storage)
      {
        core_.bind(info_,  column_, static_cast<value_storage*>(storage)->get<const char*>() );
      }
      ...
      {
        BOOST_THROW_EXCEPTION(std::runtime_error("only primitive types are supported"));
      }
