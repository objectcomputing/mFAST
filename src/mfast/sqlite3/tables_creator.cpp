...
namespace mfast
{
  namespace SQLite3 {
    ...
    tables_creator::tables_creator(insert_map_t&      insert_statements,
                                   ...
                                   bool               find_key_needed)
      : insert_statements_(insert_statements)
      , masks_(masks)
    ...
    const char* tables_creator::table_name() const
    {
    ...
    void tables_creator::add_foreign_key(std::ostream& os,
                                         const char*   tablename,
                                         const char*   key_name,
                                         const char*   key_type)
    {
      os << "  " <<  tablename << key_name << " " << key_type
         << " REFERENCES "<< tablename << "(" << key_name << "),\n";
    }
      ...
      else
      {
        ...
        if (ref) {
          ...
          tables_creator nested_builder(insert_statements_,
                                        masks_,
                                        table_name_.c_str(),
                                        primary_key_name_.c_str(),
                                        primary_key_type_.c_str());
    ...
    void tables_creator::visit(const template_instruction* inst,  void*)
    {
      ...
      if (insert_statements_.count(inst->id())) {
        std::stringstream err_msg;
        err_msg << "Duplicated template id= "<<  inst->id() << " for "<< inst->name();
        BOOST_THROW_EXCEPTION(std::runtime_error(err_msg.str()));
      }
    ...
    void tables_creator::visit(const uint32_vector_field_instruction* inst,  void*)
    {
      create_current_ << "  " << inst->name() << " INT,\n";
      parameters_.push_back(inst->name());
      num_columns_++;
    ...
    void tables_creator::visit(const uint64_vector_field_instruction* inst,  void*)
    {
      create_current_ << "  "<< inst->name() << " INT,\n";
      parameters_.push_back(inst->name());
      num_columns_++;
