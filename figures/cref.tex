...
\begin{document}
    ...
    \umlclass[width=25ex, x=-12,y=7]{int32\_cref}{}{
        + value() : int32\_t
    }
    \umlclass[width=25ex, x=-12,y=3.5]{uint32\_cref}{}{
        + value() : uint32\_t
    }
    \umlclass[width=25ex, x=-12,y=0]{int64\_cref}{}{
         + value() : int64\_t
    }
    \umlclass[width=25ex, x=-12,y=-3.5]{uint64\_cref}{}{
         + value() : uint64\_t
    }
    \umlclass[width=25ex, x=-12,y=-7]{decimal\_cref}{}{
     + value() : decimal
    }

    \umlclass[width=32ex, x=-6,y=5.25]{ascii\_string\_cref}{}{
      + size() : size\_t \\
      + c\_str() : const char* \\
      + value() : string
    }
    \umlclass[width=32ex, x=-6,y=1.75]{unicode\_string\_cref}{}{
      + size() : size\_t \\
      + c\_str() : const char* \\
      + value() : string
    }
    \umlclass[template={T},width=32ex, x=-6,y=-1.75]{vector\_cref}{}{
      + size() : size\_t \\
      + data() : const T* \\
      + value() : std::vector<T>
    }
