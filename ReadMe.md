...
mref.set_ApplVerID().present(false);
assert(!mref.get_ApplVerID().present());

// present(false) has no effect on mandatory fields
mref.set_MsgSeqNum().present(false);
assert(mref.get_ApplVerID().present());

// you can also use clear() to replace present(false)
mref.set_ApplVerID().as("C");
assert(mref.get_ApplVerID().present());
mref.set_ApplVerID().clear();
assert(!mref.get_ApplVerID().present());

// However, present(false) and clear() have different effect on
// mandatory sequence, string, or byte_vector

assert(mref.set_SenderCompID.size() == 1);
mref.set_SenderCompID.present(false); // no effect
