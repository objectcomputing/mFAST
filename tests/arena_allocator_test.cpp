...
BOOST_AUTO_TEST_SUITE( arena_allocator_test_suit )
...
BOOST_AUTO_TEST_CASE(arena_allocatore_test)
{
  ...
  void* block3 = alloc.allocate(arena_allocator::default_chunk_size/2 - 3*sizeof(intptr_t));

  // now we will get memory from another chunk
  void* block4 = alloc.allocate(arena_allocator::default_chunk_size/2);
  alloc.reset();

  void* block5 = alloc.allocate(arena_allocator::default_chunk_size - 3*sizeof(intptr_t));
  BOOST_CHECK_EQUAL(block4, block5);

  // this should get the memory from the first chunk
  void* block6 = alloc.allocate(arena_allocator::default_chunk_size - 3*sizeof(intptr_t));
